...
- Companion apps can request that same domain action through `daydrop://open-today-folder`. Compatible callers may include a stable target-display ID; after the usual authorization and safe folder preparation, DayDrop creates a Finder window on that display. The URL does not expose a filesystem path or bypass Downloads authorization, ownership checks, or managed-folder persistence. If Finder automation is unavailable or denied, DayDrop falls back to the standard system open behavior and reports that exact placement was not applied.
