...
`dist/DayDrop-1.2.1.dmg` is the current 2026-08-26 distribution artifact. It contains the universal `DayDrop.app` and an `/Applications` installation shortcut. The app and DMG are timestamped with the installed Developer ID identity; Apple notarization submission `ee3c0335-c67f-4093-b164-7538ffe0ccf1` returned `Accepted` with no reported issues, the ticket is stapled, and Gatekeeper reports `Notarized Developer ID`. Its SHA-256 is `bcc20f2ff25fa81ce8402a9b967f039ec3b6fe3a25333ca68fd625da5f112dda`. Cloudflare Pages deployment `d69686e2-acf5-420d-8b0c-de46b0841210` serves the matching homepage, signed appcast, and complete DMG at `https://daydrop.liveby.app`. A three-display installed Debug run opened the managed-day Finder window on the Mi Monitor and Studio Display; the final installed DayDrop 1.2.1 plus ForNow 0.7.7 pair repeated the actual capsule-click placement on Studio Display. WindowServer bounds confirmed each physical-screen result. The permission-denied path, display unplug/replug behavior, DayDrop-row-to-ForNow context menu, Settings visual inspection, login item, notification, broader visual, minimum-OS, and large-tree performance remain separate acceptance gates.
