...
`dist/DayDrop-1.2.0.dmg` is the current 2026-08-19 distribution artifact. It contains the universal `DayDrop.app` and an `/Applications` installation shortcut. The app and DMG are timestamped with the installed Developer ID identity; Apple notarization submission `797a2a95-b028-4fa7-aa63-0a6e9d1cb4be` returned `Accepted` with no reported issues, the ticket is stapled, and Gatekeeper reports `Notarized Developer ID`. Its SHA-256 is `d1191f696eae91b003ae91b4367a2c71ffa86ff246c7eb3924a08be36beb26ef`. The installed `daydrop://open-today-folder` action opened the current managed day folder in Finder, and the website passed desktop plus 390 px responsive checks. The DayDrop-row-to-ForNow context-menu flow, Settings visual inspection, permission, login-item, notification, broader visual, minimum-OS, and large-tree performance acceptance remain separate release gates.

Version 1.2.0 (build 8) adds the capability-gated 搁这儿-ForNow file action, the **扩展功能** Settings destination, and the narrow `daydrop://open-today-folder` action for companion apps while retaining the existing Downloads authorization and item-identity checks. `npm run release:mac` generates the notarized DMG, signed appcast, website checksum, and current-version homepage content, but intentionally does not deploy. `npm run appcast:mac` can regenerate the website release content for an already notarized artifact. The Sparkle private key is stored in the local login Keychain under `com.liuyuhang.DayDrop`; only the public key is embedded in the app.
